...
class ResponseMetadata(BaseModel):
    error: ErrorInfo = Field(default_factory=ErrorInfo, alias="Error")
...
class ModerateV2Response(BaseModel):
    ...
    result: ModerateV2Result = Field(default_factory=ModerateV2Result, alias="Result")
...
class ClientV2:
    ...
    def GenerateV2Stream(self, request):
        ...
        try:
            # 发送 HTTP 请求
            resp = self.http_client.post(self.url + "/v2/generate", data=requestBody, headers=headers, stream=True)
