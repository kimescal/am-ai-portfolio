...
class AgentClient:
    ...
    async def ainvoke(
        ...
        if thread_name:
            request.thread_name = thread_name
    ...
    def get_threads(self, user_id: str) -> list[str]:
        request = QueryThreads(user_id = user_id)
        try:
            response = httpx.post(
                f"{self.base_url}/get_threads",
                json=request.model_dump(),
                headers=self._headers,
                timeout=self.timeout,
            )
    ...
    def get_all_chats(self, admin_key: str) -> dict:
        """
        ...
        Returns:
            dict: Dictionary containing all chat thread metadata
        """
        try:
            response = httpx.get(
                f"{self.base_url}/admin/all-chats",
    ...
    def get_thread_messages(self, thread_id: str, admin_key: str) -> dict:
        """
        Get messages for a specific thread (admin only).
        
        Args:
            thread_id (str): The thread ID to get messages for
            admin_key (str): Admin authentication key
            
        Returns:
            dict: Dictionary containing thread messages
        """
        try:
            response = httpx.get(
                f"{self.base_url}/admin/thread-messages",
                params={"thread_id": thread_id, "admin_key": admin_key},
                headers=self._headers,
                timeout=self.timeout,
            )
            response.raise_for_status()
    ...
    def verify_employee(self, badge: str) -> dict:
        """
        Verify employee badge number and return employee info.

        Args:
            badge (str): Employee badge number

        Returns:
            dict: {"success": bool, "name": str} or error info
        """
        try:
            response = httpx.get(
                f"{self.base_url}/verify-employee/{badge}",
                headers=self._headers,
                timeout=self.timeout,
            )
            response.raise_for_status()
